#!/usr/bin/env python3
"""
Stage-1 dataset generator for DSENet-style training.
...
def fix_length(sig: np.ndarray, target_len: int, axis: int = -1) -> np.ndarray:
    cur_len = sig.shape[axis]
    if cur_len > target_len:
        slicer = [slice(None)] * sig.ndim
        slicer[axis] = slice(0, target_len)
        return sig[tuple(slicer)]
    elif cur_len < target_len:
        pad_width = [(0, 0)] * sig.ndim
        pad_width[axis] = (0, target_len - cur_len)
        return np.pad(sig, pad_width, mode="constant")
    return sig
...
def sample_doas_with_min_separation(
    ...
    """
    Sample DOAs in [0, 360) with at least min_sep_deg separation.
    """
    if n_sources * min_sep_deg > 360:
        raise ValueError(
            f"Impossible to place {n_sources} sources with {min_sep_deg} deg separation."
        )
    ...
    for _ in range(max_attempts):
        ...
        if all(angular_distance_deg(candidate, existing) >= min_sep_deg for existing in doas):
            doas.append(candidate)

            if len(doas) == n_sources:
                return doas

    raise RuntimeError(
        f"Could not sample {n_sources} DOAs with min separation {min_sep_deg} deg."
    )
...
def max_radius_in_room_for_angle(
    ...
    """
    Maximum valid radius from mic_center along theta_deg while staying inside room.
    """
    ...
    if not radii:
        raise RuntimeError(f"Could not compute valid radius for DOA {theta_deg}")

    return min(radii)


def source_position_from_doa_fixed_radius(
    room_dim: np.ndarray,
    mic_center: np.ndarray,
    theta_deg: float,
    cfg: GenConfig,
    ...
    """
    Place source at fixed radius from mic center according to DOA.
    If the fixed radius is too large for the sampled room/angle,
    use the largest valid radius.
    """
    ...
    r = min(cfg.source_radius_m, 0.9 * r_max)
    ...
    # Fixed height reduces unwanted variation
    z = cfg.mic_height_m
...
class Stage1DatasetBuilder:
    def __init__(self, cfg: GenConfig):
        ...
        self.target_len = int(round(cfg.clip_seconds * cfg.sample_rate))
    ...
    def _sample_scene_audio(self) -> Tuple[List[np.ndarray], np.ndarray]:
        ...
        for p in speech_paths:
            ...
            sig = crop_or_tile(sig, self.target_len)
        ...
        noise = crop_or_tile(noise, self.target_len)
    ...
    def _generate_scene(self, scene_id: int) -> Tuple[np.ndarray, List[np.ndarray], List[int]]:
        ...
        # Sample DOAs first, then place speakers at fixed radius.
        speaker_doas_float = sample_doas_with_min_separation(
            n_sources=cfg.n_speakers,
            min_sep_deg=cfg.min_doa_sep_deg,
        )

        spk_positions = [
            source_position_from_doa_fixed_radius(
                room_dim=room_dim,
                mic_center=mic_center,
                theta_deg=doa,
                cfg=cfg,
            )
            for doa in speaker_doas_float
        ]

        # Noise can remain random, or you can remove noise if not needed.
        noise_position = sample_source_position(room_dim, cfg)

        # Integer DOAs for filenames
        ...
        mixture_mc = fix_length(mixture_mc, self.target_len, axis=1)
        ...
        for i in range(cfg.n_speakers):
            ...
            target_ref = fix_length(target_ref, self.target_len, axis=0)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output_root", type=str, default=r"D:\邵鹏远\UCL\博1\code\DSENet\data\dataset_4mic_6spk", help="Output dataset root")

    parser.add_argument("--train_items", type=int, default=14400, help="Number of stage-1 trainer items for train")
    parser.add_argument("--val_items", type=int, default=3600, help="Number of stage-1 trainer items for val")
    parser.add_argument("--test_items", type=int, default=2000, help="Number of stage-1 trainer items for test")
...
def main() -> None:
    ...
    builder.build_split(SplitConfig(name="train", num_items=args.train_items))
    builder.build_split(SplitConfig(name="val", num_items=args.val_items))
    builder.build_split(SplitConfig(name="test", num_items=args.test_items))
