...
def plot_curve(doas, values, title, xlabel, ylabel, save_path, gt_doa=None):
    ...
    plt.plot(doas, values, linewidth=2, label="SI-SDRi")
    if gt_doa is not None:
        plt.axvline(gt_doa, linestyle="--", linewidth=1.5, label=f"GT DOA = {gt_doa}°")
    plt.xlim(0, 359)
...
def main():
    ...
    plot_curve(
        doas=doa_list,
        values=sisdri_list,
        title=f"DOA Sweep SI-SDRi (fixed width = {fixed_width}°)",
        xlabel="Input DOA φ(°)",
        ylabel="SI-SDRi (dB)",
        save_path=plot_path,
        gt_doa=gt_doa
